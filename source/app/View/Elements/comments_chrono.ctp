...
<?php if ( isset($data['cmt_id']) && $data['cmt_id'] && !empty($subject) ): ?>
    <div class="comment_view-all">
        <span class="comment_view-icon material-icons moo-icon moo-icon-question_answer">question_answer</span><a href="<?php echo $subject[key($subject)]['moo_href'];?>" class="showAllComments"><?php echo __('View all comments')?></a>
    </div>
<?php else:?>
