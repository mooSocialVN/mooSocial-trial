<div>
	...
	?>						
	<?php $this->getEventManager()->dispatch(new CakeEvent('element.comments.afterShowCommentMessage', $this, array('comment' => $activity_comment))); ?>
	...
	<div class="comment_thumb">
		<a data-dismiss="modal" href="<?php echo $this->Moo->getImageUrl($activity_comment,array());?>">
			<?php if($this->Moo->isGifImage($this->Moo->getImageUrl($activity_comment,array()))) :  ?>
                        <?php echo $this->Moo->getImage($activity_comment,array('class'=>'comment-img gif_image'));?>
                    <?php else: ?>
                        <?php echo $this->Moo->getImage($activity_comment,array('class'=>'comment-img', 'prefix'=>'200'));?>
                    <?php endif; ?>
		</a>
	</div> 
	<?php endif;?>
    ...
    <?php endif; ?>
	<script>
		$('#history_activity_comment_<?php echo $activity_comment['ActivityComment']['id']?>').find('.history_activity_comment-text').html("<?php echo addslashes(__('Edited')).(isset($other_user) ? ' '.addslashes(__('by')).' '.$other_user['name'] : '');?>");
	</script>
</div>