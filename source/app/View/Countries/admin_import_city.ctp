   ...
   $(document).ready(function(){
      ...
      $('#createButton').click(function(){
            if($('#filename').val() != ''){
                ...
                $.post("<?php echo  $this->request->base ?>/admin/countries/getcsvcities", $("#frm_getcsv").serialize(), function (data) {
...
<div class="modal-header">
    ...
    <h4 class="modal-title"><?php echo __('Import City');?></h4>
</div>
             ...
             <div id="photos_upload"></div>
                    <div id="attachments_upload"></div>                                     
                     <form id='frm_getcsv' method="post" enctype="multipart/form-data">
                        ...
                        <input type="hidden" id="country_id" name="country_id" value="<?php echo $state['Country']['id'] ?>">
                        <input type="hidden" id="state_id" name="state_id" value="<?php echo $state['State']['id'] ?>">                       
                </form>
            <div>
                <?php echo __('You can upload a text file with a list of city that you would like to import and each state/province should be on a new line.'); ?>
            </div>
        </div>
