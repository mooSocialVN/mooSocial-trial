<script>
    $(document).ready(function() {
        ...
        $('#tCreateButton').click(function() {
            ...
            $.post("<?php echo  $this->request->base ?>/admin/profile_fields/ajax_translate_save_option", $("#tCreateForm").serialize(), function(data) {
...
<div class="modal-body">
    <form id="tCreateForm" class="form-horizontal" role="form">
        <?php echo $this->Form->hidden('id', array('value' => $field_option['ProfileFieldOption']['id'])); ?>
        <?php foreach ($languages as $key => $language) : ?>
            ...
            <div class="form-body">
                <div class="form-group">
                    ...
                    <div class="col-md-9">
                        <?php foreach ($field_option['nameTranslation'] as $translation) : ?>
                            <?php if ($translation['locale'] == $key) : ?>
                                <?php $lval = $translation['content'];
                                break; ?>
                            <?php endif; ?>
                        <?php endforeach; ?>
                        <?php echo $this->Form->text('name.' . $key, array('placeholder' => __('Enter text'), 'class' => 'form-control', 'value' => $lval)); ?>
                    </div>
                </div>
            </div>
<?php endforeach; ?>
